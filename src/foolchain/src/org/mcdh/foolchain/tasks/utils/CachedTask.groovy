package org.mcdh.foolchain.tasks.utils
...
class CachedTask extends DefaultTask {
 private static class Annotated {
  ...
  private AnnotatedElement getElement() throws Throwable {
   if (isMethod) {
  ...
  protected Object getValue(Object instance) throws Throwable {
   ...
   if (isMethod) {
    ...
    final char[] name = symbolName.toCharArray()
 ...
 CachedTask() {
  ...
  onlyIf {
   Task task ->
    if (!doesCache() || cachedList.isEmpty()) {
     return true
    }
    for (Annotated field : cachedList) {
     try {
      ...
      if (!hashFile.exists()) {
       // Kill the output file if the hash doesn't exist, else gradle will think it's up-to-date
       file.delete()
       return true
      }
 ...
 private void addCachedOutput(final Annotated ann) {
  cachedList.add(ann)
  ...
  this.doLast(new Action<Task>()
  {
   ...
   void execute(Task task) {
    ...
    try {
     File outFile = getProject().file(ann.getValue(task))
     if (outFile.exists()) {
      File hashFile = getHashFile(outFile)
      Files.write(getHashes(ann, inputList, task), hashFile, Charset.defaultCharset())
     }
    }
    // error? spit it and do the task.
    catch (Exception e) {
 ...
 private String getHashes(Annotated output, List<Annotated> inputs, Object instance) throws Throwable {
