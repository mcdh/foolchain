...
import org.mcdh.toolchain.utils.json.version.AssetIndex

import java.util.concurrent.ConcurrentLinkedQueue

class DownloadAssetsTask extends DefaultTask {
 ...
 public def assetsDir
 public AssetIndex index
 public String indexName

 private boolean errored = false
 private File virtualRoot = null
 private final Queue<Asset> filesLeft = new ConcurrentLinkedQueue<>()
  ...
  void run() {
   ...
   while ((asset = filesLeft.poll()) != null) {
    for (int i = 1; i < MAX_TRIES + 1; i++) {
     try {
      File file = new File(project.file(assetsDir), "objects/" + asset.path)
 ...
 @TaskAction
 def doTask() {
  final File assetsDir = project.file(this.assetsDir)
  File out = new File(assetsDir, "objects")
  out.mkdirs()
